...
export class PortalIntegrationsService {
  ...
  async handleIndiaMART(raw: any) {
    ...
    const payload: PortalLeadPayload = {
      ...
      metadata: {
        portal: 'IndiaMART',
        rawProduct: raw.product_name || raw.product || raw.PRODUCT_NAME,
        rawCategory: raw.category || raw.CATEGORY,
        rawCompanyName: raw.buyer_company || raw.company || raw.COMPANY_NAME,
        rawLeadId: raw.lead_id || raw.enquiry_id || raw.LEAD_ID,
        rawSource: raw.source || raw.lead_source,
        rawPayload: raw,
      },
    };
    ...
    await this.prisma.webhookEvent.create({
      data: { provider: 'indiamart', eventType: 'portal_lead', idempotencyKey: key, rawPayload: raw, processedResult: result },
    });
  ...
  async handle99Acres(raw: any) {
    ...
    const payload: PortalLeadPayload = {
      ...
      metadata: {
        portal: '99acres',
        rawPropertyType: raw.property_type || raw.PROPERTY_TYPE,
        rawPropertyFor: raw.property_for || raw.PROPERTY_FOR,
        rawLocation: raw.location || raw.LOCATION,
        rawListingId: raw.listing_id || raw.property_id || raw.LISTING_ID,
        rawBedrooms: raw.bedrooms || raw.BEDROOMS,
        rawPostedBy: raw.posted_by || raw.POSTED_BY,
        rawPayload: raw,
      },
    };
    ...
    const result = { contact, lead };
    await this.prisma.webhookEvent.create({
      data: { provider: '99acres', eventType: 'portal_lead', idempotencyKey: key, rawPayload: raw, processedResult: result },
    });
  ...
  async handleJustDial(raw: any) {
    ...
    const payload: PortalLeadPayload = {
      ...
      metadata: {
        portal: 'JustDial',
        rawServiceCategory: raw.service_category || raw.SERVICE_CATEGORY,
        rawBusinessName: raw.business_name || raw.BUSINESS_NAME,
        rawLocation: raw.location || raw.LOCATION,
        rawEnquiryId: raw.enquiry_id || raw.ENQUIRY_ID,
        rawPayload: raw,
      },
    };
    ...
    const result = { contact, lead };
    await this.prisma.webhookEvent.create({
      data: { provider: 'justdial', eventType: 'portal_lead', idempotencyKey: key, rawPayload: raw, processedResult: result },
    });
  ...
  async handleMagicBricks(raw: any) {
    ...
    const payload: PortalLeadPayload = {
      ...
      city: raw.city || raw.CITY,
      budget: raw.budget || raw.min_budget || raw.max_budget || raw.BUDGET,
      requirement: raw.property_type || raw.listing_type || raw.PROPERTY_TYPE,
      metadata: {
        portal: 'MagicBricks',
        rawPropertyType: raw.property_type || raw.PROPERTY_TYPE,
        rawListingId: raw.listing_id || raw.property_id || raw.LISTING_ID,
        rawLocation: raw.location || raw.locality || raw.LOCATION,
        rawPostedBy: raw.posted_by || raw.POSTED_BY,
        rawBedrooms: raw.bedrooms || raw.BEDROOMS,
        rawPayload: raw,
      },
    ...
    const result = { contact, lead };
    await this.prisma.webhookEvent.create({
      data: { provider: 'magicbricks', eventType: 'portal_lead', idempotencyKey: key, rawPayload: raw, processedResult: result },
    });
  ...
  async handleHousing(raw: any) {
    ...
    const result = { contact, lead };
    await this.prisma.webhookEvent.create({
      data: { provider: 'housing', eventType: 'portal_lead', idempotencyKey: key, rawPayload: raw, processedResult: result },
    });
  ...
  async handleTradeIndia(raw: any) {
    ...
    const payload: PortalLeadPayload = {
      ...
      city: raw.city || raw.buyer_city || raw.CITY,
      budget: raw.budget || raw.BUDGET,
    ...
    const result = { contact, lead };
    await this.prisma.webhookEvent.create({
      data: { provider: 'tradeindia', eventType: 'portal_lead', idempotencyKey: key, rawPayload: raw, processedResult: result },
    });
