    ...
    switch (pack.type) {
      case 'custom_fields':
        for (const f of payload.fields || []) {
          const created = await this.prisma.customFieldDefinition.create({ data: { ...f, tenantId, active: true } });
          add('customFieldDefinition', created.id);
        }
        break;
      case 'pipeline_stages':
        for (const s of payload.stages || []) {
          const created = await this.prisma.pipelineStage.create({ data: { ...s, tenantId } });
      ...
      case 'campaigns':
        for (const c of payload.campaigns || []) {
          const created = await this.prisma.campaign.create({
            data: { ...c, tenantId, creatorId: userId, assignedAgentId: userId } as any,
          });
      ...
      case 'lead_forms':
        for (const f of payload.forms || []) {
          ...
          const created = await this.prisma.leadForm.create({ data: { ...formData, tenantId } });
      ...
      case 'message_templates':
        for (const t of payload.templates || payload.messages || []) {
          const created = await this.prisma.messageTemplate.create({ data: { ...t, tenantId, creatorId: userId } as any });
          add('messageTemplate', created.id);
        }
        break;
      case 'scoring_rules':
        for (const r of payload.rules || []) {
          const created = await this.prisma.scoringRule.create({ data: { ...r, tenantId } });
          add('scoringRule', created.id);
        }
        break;
      case 'routing_rules':
        for (const r of payload.rules || []) {
          const created = await this.prisma.routingRule.create({ data: { ...r, tenantId, active: true } as any });
          add('routingRule', created.id);
        }
        break;
      case 'automation_rules':
        for (const r of payload.rules || []) {
          const created = await this.prisma.automationRule.create({ data: { ...r, tenantId, active: true } as any });
      ...
      case 'nurture_sequences':
        for (const seq of payload.sequences || []) {
          ...
          const created = await this.prisma.nurtureSequence.create({ data: { ...seqData, tenantId } as any });
      ...
      case 'booking_settings':
        for (const b of payload.settings || []) {
          const created = await this.prisma.bookingSetting.create({ data: { ...b, tenantId } as any });
          add('bookingSetting', created.id);
        }
        break;
      case 'crm_mappings':
        for (const m of payload.mappings || []) {
          const created = await this.prisma.crmMapping.create({ data: { ...m, tenantId } as any });
          add('crmMapping', created.id);
        }
        break;
      case 'conversion_goals':
        // skipped — conversions require a real leadId
