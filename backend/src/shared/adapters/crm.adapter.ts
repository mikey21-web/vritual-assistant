...
export class HubspotAdapter implements CrmAdapter {
  async pushLead(lead: any, mapping: any): Promise<{ success: boolean; externalId?: string; error?: string }> {
    const apiKey = mapping.config?.apiKey;
...
export class SalesforceAdapter implements CrmAdapter {
  async pushLead(lead: any, mapping: any): Promise<{ success: boolean; externalId?: string; error?: string }> {
    const clientId = mapping.config?.clientId;
    const clientSecret = mapping.config?.clientSecret;
    const username = mapping.config?.username;
    const password = mapping.config?.password;
...
export class ZohoAdapter implements CrmAdapter {
  async pushLead(lead: any, mapping: any): Promise<{ success: boolean; externalId?: string; error?: string }> {
    const refreshToken = mapping.config?.refreshToken;
    const clientId = mapping.config?.clientId;
    const clientSecret = mapping.config?.clientSecret;
    ...
    try {
      ...
      if (json.data?.[0]?.code === 'SUCCESS') return { success: true, externalId: json.data[0]?.details?.id };
