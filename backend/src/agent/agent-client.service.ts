...
export class AgentClientService {
  ...
  async trigger(tenantId: string, leadId: string, triggerId: string, channel: string, messageText: string, triggerType = 'inbound_message') {
    ...
    try {
      await firstValueFrom(
        this.http.post(`${url}/agent/run`, {
          tenantId, leadId, triggerId, channel,
          trigger: triggerType,
          messageText,
        }, {
