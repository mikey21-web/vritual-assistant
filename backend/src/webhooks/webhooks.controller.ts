...
export class WebhooksController {
  constructor(private service: WebhooksService, private security: WebhookSecurityService) {}

  @Post('forms') @HttpCode(200) @ApiOperation({ summary: 'Receive form submission webhook (API key auth)' })
  formWebhook(@Body() d: FormWebhookDto, @Headers('x-api-key') apiKey?: string) {
  ...
  whatsappWebhook(@Body() d: WhatsAppWebhookDto, @Headers('x-hub-signature-256') signature?: string, @Req() req?: RawBodyRequest<Request>) {
  ...
  socialWebhook(@Body() d: GenericWebhookDto, @Headers('x-api-key') apiKey?: string) {
  ...
  callWebhook(@Body() d: GenericWebhookDto, @Headers('x-api-key') apiKey?: string) {
  ...
  paymentWebhook(@Body() d: GenericWebhookDto, @Headers('stripe-signature') signature?: string, @Req() req?: RawBodyRequest<Request>) {
    const rawBody = req?.rawBody ?? Buffer.from(JSON.stringify(d));
  ...
  chatbotWebhook(@Body() d: GenericWebhookDto, @Headers('x-api-key') apiKey?: string) {
  ...
  mobileAppWebhook(@Body() d: GenericWebhookDto, @Headers('x-api-key') apiKey?: string) {
