...
@Injectable()
export class JarvisSchedulerService implements OnApplicationBootstrap {
  private readonly logger = new Logger(JarvisSchedulerService.name);
  ...
  onApplicationBootstrap(): void {
    this.logger.log('Jarvis Scheduler starting — scanning every 5 minutes');
    this.scan();
    ...
    try {
      ...
      for (const finding of newFindings) {
        await this.events.emit({
          type: `jarvis.${finding.type}`,
          source: 'jarvis-scheduler',
