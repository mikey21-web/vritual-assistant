...
export function StatCard({
  ...
  return (
    <div id={id || `stat-${title.toLowerCase().replace(/\s+/g, '-')}`}
      className={`relative rounded-xl border border-[var(--border)] bg-[var(--card)] p-5 transition-all
        ${isHero ? 'md:col-span-2 lg:col-span-3 p-6' : ''}`}
    >
      <div className="flex items-start justify-between">
        <span className="text-[11px] font-mono uppercase tracking-wider text-[var(--muted-foreground)] font-semibold">{title}</span>
          ...
          <div className="shrink-0 pt-0.5 ml-4">
            <svg width="80" height="24" className="overflow-visible">
              <path d={generateSparklinePath(sparklineData)} fill="none"
                stroke={changeDirection === 'up' ? '#10b981' : changeDirection === 'down' ? '#e11d48' : '#94a3b8'}
      ...
      <div className="mt-3 flex items-baseline justify-between gap-x-2">
        <span className={`font-mono tracking-tight font-medium text-[var(--foreground)] ${isHero ? 'text-3xl sm:text-4xl' : 'text-2xl sm:text-3xl'}`}>
          {value}
        </span>
        {change && (
          <div className="flex items-center gap-1 text-[13px]">
            ...
            <span className="text-[var(--muted-foreground)] text-xs">{timeframe}</span>
          </div>
        )}
      </div>
      {isHero && (
