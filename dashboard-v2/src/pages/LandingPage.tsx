...
import {
  ...
  Webhook, QrCode, Send, Building2, Calendar, FileText,
} from 'lucide-react';
...
const nicheContent: Record<string, {
  ...
  healthcare: {
    ...
    description: 'AI assistant that handles appointment booking, FAQs, and patient communication — all over WhatsApp. No call waiting, no missed patients.',
    ...
    features: [
      { icon: MessageCircle, title: 'WhatsApp patient communication', desc: 'Appointment reminders, follow-ups, and notifications — all on WhatsApp.' },
      { icon: Bot, title: 'AI patient assistant', desc: 'Answers FAQs (timings, fees, doctor availability). Books appointments 24/7.' },
      { icon: Calendar, title: 'Appointment scheduling', desc: 'Patients book from WhatsApp or website. Auto-confirm and reschedule.' },
      { icon: Target, title: 'Smart patient routing', desc: 'Route patients to the right department. Track appointment types and insurance info.' },
      { icon: BarChart3, title: 'Clinic analytics', desc: 'Appointment volume, department performance, patient sources. Data-driven insights.' },
      { icon: Shield, title: 'Consent & compliance', desc: 'Built-in consent management. DPDP compliant. Full audit trail.' },
    ],
    painPoints: ['Phone rings constantly for appointment bookings', 'Patients forget appointments — high no-show rate', 'Same questions answered repeatedly', 'No unified view of patient pipeline', 'Walk-ins vs scheduled — no coordination'],
    solutions: ['AI books appointments 24/7', 'Auto-reminders cut no-shows', 'AI handles FAQ instantly', 'Pipeline view from inquiry to completed', 'Department-wise routing and tracking'],
  },
  "event-marketing-agency": {
    name: 'EventPro',
    tagline: 'Event Planning Lead Platform',
    hero: ['Event inquiries', 'get a proposal', 'before the coffee cools'],
    heroHighlight: 'before the coffee cools',
    description: 'AI agent that handles event inquiries, qualifies budget and preferences, sends package details, books consultation calls, and creates event drafts. No more missed opportunities.',
    badge: 'Event Lead Automation',
    channels: [
      { label: 'WhatsApp', icon: MessageCircle, color: '#25D366' },
      { label: 'Website Widget', icon: Globe, color: '#2563eb' },
      { label: 'SMS', icon: Smartphone },
    ...
    features: [
      { icon: MessageCircle, title: 'WhatsApp-first lead capture', desc: 'Clients inquire about events on WhatsApp. AI responds instantly, qualifies event type, guest count, and budget.' },
      { icon: Bot, title: 'AI client qualification', desc: 'Extracts event type, guest count, budget, venue preference, services needed. Scores and segments automatically.' },
      { icon: Calendar, title: 'Consultation scheduler', desc: 'Syncs with your calendar. Sends available slots for consultation calls and venue visits.' },
      { icon: Target, title: 'Event pipeline', desc: 'Track every lead from inquiry → proposal → deposit → confirmed event. Real-time pipeline visibility.' },
      { icon: BarChart3, title: 'Agency performance', desc: 'Proposal conversion rates, lead response times, average deal size. Data-driven selling.' },
      { icon: FileText, title: 'Auto event drafts', desc: 'AI creates event drafts in the system. No manual data entry needed.' },
    ],
    painPoints: ['Leads message at night — no reply till morning', 'Proposals take days to prepare', 'Follow-ups slip through cracks', 'No unified view of active deals', 'Manual data entry into systems'],
    solutions: ['AI responds in seconds, 24/7', 'AI qualifies and scores instantly', 'Automated follow-up sequences', 'Pipeline view from inquiry to booked', 'Auto-creates event drafts'],
  },
  logistics: {
    name: 'FreightConnect',
    ...
    features: [
      { icon: MessageCircle, title: 'WhatsApp shipment inquiries', desc: 'Shippers ask for quotes, book shipments, and track status — all on WhatsApp.' },
      { icon: Bot, title: 'AI quoting agent', desc: 'Asks origin/destination, weight, timeline. Generates instant quotes. Books directly.' },
      { icon: Calendar, title: 'Pickup scheduling', desc: 'Capture preferred pickup dates. Schedule and track in one place.' },
      { icon: Target, title: 'Shipment pipeline', desc: 'Track every inquiry from quote → booked → picked up → delivered. Pipeline visibility.' },
      { icon: BarChart3, title: 'Operations insights', desc: 'Quote conversion rates, shipment volume, status distribution.' },
      { icon: Link, title: 'Carrier tracking', desc: 'Assign carriers to shipments. Store carrier info and tracking numbers.' },
    ],
    painPoints: ['Quote requests pile up — slow response loses business', 'Back-and-forth calls for shipment details', 'No record of inquiry-to-booking conversion', 'Manual carrier assignment tracking', 'Delivery status requires phone calls'],
    solutions: ['AI quotes instantly, 24/7', 'WhatsApp-based self-service booking', 'Pipeline tracking from inquiry to delivery', 'Carrier assignment with tracking numbers', 'Real-time status updates on WhatsApp'],
  },
};
