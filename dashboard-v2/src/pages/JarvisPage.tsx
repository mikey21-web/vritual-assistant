  ...
  const fetchAll = async () => {
    ...
    try {
      const [s, o, i, st, a] = await Promise.all([
        api('/jarvis/status').catch(() => null),
        api('/jarvis/outcomes').catch(() => []),
        api('/jarvis/temporal-insights').catch(() => []),
        api('/jarvis/staff').catch(() => []),
        api('/jarvis/actions').catch(() => []),
      ]);
      setStatus(s);
      ...
      toast.error('Failed to load Jarvis data');
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-5">
              ...
              <div className="space-y-3">
                ...
                {(!status.outcomes || status.outcomes.length === 0) && (
                  <p className="text-sm text-[var(--muted-foreground)]">No outcomes defined yet. Tell Jarvis what you want to achieve.</p>
                )}
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-5">
              ...
              <p className="text-sm text-[var(--muted-foreground)]">
                Jarvis scans every 5 minutes for stale leads, conversion anomalies, and source shifts. Critical findings auto-trigger actions.
              </p>
              ...
              <p className="text-xs text-[var(--muted-foreground)] mt-3">
                Findings emit via Socket.IO on <code className="text-[var(--primary)]">jarvis.*</code> channels.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {outcomes.length === 0 && (
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-8 text-center">
              ...
              <p className="text-[var(--muted-foreground)]">No outcomes yet. Tell Jarvis a goal!</p>
            </div>
          )}
          {outcomes.map((o: any) => (
