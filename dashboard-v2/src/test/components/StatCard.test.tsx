...
describe('StatCard', () => {
  ...
  it('shows trend up indicator', () => {
    render(<StatCard title="Leads" value={50} trend={15} trendLabel="up" />);
    expect(screen.getByText('15%')).toBeInTheDocument();
  });

  it('shows trend down indicator', () => {
    render(<StatCard title="Leads" value={50} trend={-8} trendLabel="down" />);
    expect(screen.getByText('8%')).toBeInTheDocument();
  });

  it('shows timeframe text when provided', () => {
    render(<StatCard title="Leads" value={50} timeframe="vs last month" />);
