...
conn.on('ready', async () => {
  console.log('Building (keep npm cache)...');
  const r = await run('cd /opt/lead-automation-demo && docker build --no-cache -t lead-automation-demo-dashboard -f dashboard-v2/Dockerfile dashboard-v2/ 2>&1 | tail -5');
  console.log(r);
