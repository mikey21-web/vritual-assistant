...
conn.on('ready', async () => {
  ...
  console.log('Build...');
  r = await run('cd /opt/lead-automation-demo && docker build --no-cache --progress=plain -t lead-automation-demo-dashboard -f dashboard-v2/Dockerfile dashboard-v2/ 2>&1');
  const lines = r.split('\n');
  const errors = lines.filter(l => l.includes('ERROR') || l.includes('error') || l.includes('failed'));
  if (errors.length > 0) {
    console.log('BUILD FAILED:');
    errors.slice(0, 10).forEach(e => console.log(e));
  } else {
    console.log('BUILD OK. Last lines:');
