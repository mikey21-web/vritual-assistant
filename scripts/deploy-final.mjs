...
conn.on('ready', async () => {
  ...
  const g = await run('cd /opt/lead-automation-demo && git fetch origin master && git reset --hard origin/master 2>&1');
  console.log(g.slice(0, 200));

  console.log('Build...');
  const b = await run('cd /opt/lead-automation-demo && docker build --no-cache -t lead-automation-demo-dashboard -f dashboard-v2/Dockerfile dashboard-v2/ 2>&1 | tail -5');
  if (b.includes('ERROR')) {
    console.log('BUILD FAILED:', b);
    conn.end();
    return;
  }
...
.connect({ host: HOST, port: 22, username: 'root', password: PASS, readyTimeout: 600000, keepaliveInterval: 15000 });
