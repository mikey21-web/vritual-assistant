...
conn.on('ready', async () => {
  ...
  r = await run('cd /opt/lead-automation-demo && git log --oneline -1 2>&1');
  console.log('HEAD:', r);

  console.log('Build...');
  r = await run('cd /opt/lead-automation-demo && docker build --no-cache -t lead-automation-demo-dashboard -f dashboard-v2/Dockerfile dashboard-v2/ 2>&1');
  console.log(r.slice(-300));
